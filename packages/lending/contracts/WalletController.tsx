...
import './interfaces/IWalletController.tsol';
...
import './MarketsAggregator.tsx';

contract WalletController is Roles, IWalletController, IUpgradeableContract {
    ...
    function transferTokensToWallet(address venomWallet, address tokenRoot, address userTip3Wallet, uint256 toPayout) external override view onlyTrusted {
        TvmCell empty;
        _transferTokensToWallet(venomWallet, tokenRoot, userTip3Wallet, uint128(toPayout), empty);
    }

    function _transferTokensToWallet(address venomWallet, address tokenRoot, address userTip3Wallet, uint128 toTransfer, TvmCell payload) internal view {
    ...
    modifier onlyMarket() {
        require(msg.sender == marketAddress, ErrorCodes.ERROR_MSG_SENDER_IS_NOT_MARKET);
        _;
    }
    ...
    modifier onlyOwnWallet(address tokenRoot, address tokenWallet) {
        require(wallets[tokenRoot] == tokenWallet, ErrorCodes.ERROR_MSG_SENDER_IS_NOT_OWN_WALLET);
        _;
    }
    ...
    modifier onlyExisingTIP3Root(address rootAddress) {
        require(wallets.exists(rootAddress), ErrorCodes.ERROR_TIP3_ROOT_IS_UNKNOWN);
        _;
    }
