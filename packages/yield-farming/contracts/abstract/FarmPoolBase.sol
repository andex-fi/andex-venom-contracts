...
abstract contract FarmPoolBase is FarmPoolStorage {
    ...
    function getDetails() external virtual view responsible returns (Details) {
        return {
            ...
            rewardTokenBalanceCumulative, unclaimedReward, owner, manager, user_data_version, pool_version
        );
    }
    ...
    */
    function setUp() internal virtual {
        ...
        ITokenRoot(tokenRoot).deployWallet{
            ...
            callback: FarmPoolBase.receiveTokenWalletAddress
        }(
            address(this), // owner
        ...
        for (uint i = 0; i < rewardTokenRoot.length; i++) {
            ITokenRoot(rewardTokenRoot[i]).deployWallet{
                value: TOKEN_WALLET_DEPLOY_VALUE,
                callback: FarmPoolBase.receiveTokenWalletAddress
            }(
    ...
    function requestUpdateUserDataCode(address send_gas_to) external virtual onlyOwner {
        ...
        IManager(manager).processUpdatePoolUserDataRequest{value: 0, flag: MsgFlag.ALL_NOT_RESERVED}(send_gas_to);
    }
    ...
    function requestUpgradePool(address send_gas_to) external virtual onlyOwner {
        ...
        IManager(manager).processUpgradePoolRequest{value: 0, flag: MsgFlag.ALL_NOT_RESERVED}(send_gas_to);
    }
    ...
    */
    ...
    // withdraw all staked tokens without reward in case of some critical logic error / insufficient VENOMs on FarmPool balance
    ...
    onBounce(TvmSlice slice) external virtual {
        ...
        if (functionId == tvm.functionId(UserData.processDeposit)) {
            ...
            for (uint i = 0; i < rewardTokenRoot.length; i++) {
                ...
                ITokenRoot(rewardTokenRoot[i]).deployWallet{value: TOKEN_WALLET_DEPLOY_VALUE, callback: FarmPoolBase.dummy}(
                    deposit.user,
                    TOKEN_WALLET_DEPLOY_GRAMS_VALUE // deploy grams
                );
            }
