...
contract UserData is IUserData {
    ...
    uint32 vestingRatio; // number from 0 to 1000 (0% to 100%). 0 means vesting is disabled
    ...
    function _rangeIntervalAverage(
        uint32 interval,
        uint32 max
    ...
    function _computeVestedForInterval(
        uint128 _entitled,
        uint32 _interval
    ...
    function _computeVestedForNewlyEntitled(
        ...
        uint32 _farmEndTime
    ) internal view returns (uint128 _vested) {
        if (_entitled == 0) {
            return 0;
        }
        if (_farmEndTime == 0 || _poolLastRewardTime < _farmEndTime) {
            ...
            if (age > vestingPeriod) {
                (uint128 _vested_part, ) = _computeVestedForInterval(_entitled, age);
                return _vested_part;
            } else {
