...
abstract contract DexContractBase  {
    ...
    modifier onlyAccount(address account_owner) {
        require(msg.sender == _expectedAccountAddress(account_owner), Errors.NOT_ACCOUNT);
        _;
    }

    modifier onlyPair(address left_root, address right_root) {
        require(msg.sender == _expectedPairAddress(left_root, right_root), Errors.NOT_PAIR);
        _;
    }
