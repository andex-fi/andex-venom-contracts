    ...
    INextExchangeData
{
    ...
    function onAcceptTokensTransfer(
        ...
        if (errorCode == 0) {
            if (msg.sender == lp_wallet) {
                if (op == OperationTypes.WITHDRAW_LIQUIDITY || op == OperationTypes.WITHDRAW_LIQUIDITY_V2) {
                    ...
                    if (!operationsOpt.hasValue()) {
                        ...
                        IPairOperationCallback(sender_address).dexPairWithdrawSuccess{
                            value: Constants.OPERATION_CALLBACK_BASE + 30,
                        ...
                        if (recipient != sender_address) {
                            IPairOperationCallback(recipient).dexPairWithdrawSuccess{
                        ...
                        for (uint8 ii = 0; ii < N_COINS; ii++) {
                            if (operations[ii].amount >= 0) {
                                ITokenVault(_expectedTokenVaultAddress(operations[ii].root)).transfer{
                ...
                if (op == OperationTypes.EXCHANGE || op == OperationTypes.EXCHANGE_V2) {
                    ...
                    if (!dy_result_opt.hasValue()){
                        ...
                        IPairOperationCallback(sender_address).dexPairExchangeSuccess{
                        ...
                        if (recipient != sender_address) {
                            IPairOperationCallback(recipient).dexPairExchangeSuccess{
                                value: Constants.OPERATION_CALLBACK_BASE,
                                flag: MsgFlag.SENDER_PAYS_FEES + MsgFlag.IGNORE_ERRORS,
                                bounce: false
                            }(id, false, IExchangeResult.ExchangeResult(
                    ...
                    if (errorCode == 0) {
                        if (msg.value < (Constants.CROSS_POOL_EXCHANGE_MIN_VALUE + referrer_value) * (1 + all_nested_nodes)) {
                    ...
                    if (errorCode == 0) {
                        ...
                        IPairOperationCallback(sender_address).dexPairExchangeSuccess{
                            value: Constants.OPERATION_CALLBACK_BASE + 40,
                            flag: MsgFlag.SENDER_PAYS_FEES + MsgFlag.IGNORE_ERRORS,
                            bounce: false
                        }(id, false, IExchangeResult.ExchangeResult(
                            i == 0 && j == 1,
                        ...
                        for (uint32 idx = 0; idx < next_steps.length; idx++) {
                            ...
                            IBasePool(next_step.poolRoot).crossPoolExchange{
                                value: idx == max_nested_nodes_idx ? 0 : (next_step.nestedNodes + 1) * (Constants.CROSS_POOL_EXCHANGE_MIN_VALUE + referrer_value) + current_extra_value,
                                flag: idx == max_nested_nodes_idx ? MsgFlag.ALL_NOT_RESERVED : MsgFlag.SENDER_PAYS_FEES
                            }(
                                id,

                                current_version,
                                PoolTypes.STABLESWAP,
    ...
    function _checkOperationData(
        ...
        if (!(msg_sender == lp_wallet && (op == OperationTypes.WITHDRAW_LIQUIDITY || op == OperationTypes.WITHDRAW_LIQUIDITY_V2) ||
        ...
        if ((op == OperationTypes.WITHDRAW_LIQUIDITY || op == OperationTypes.WITHDRAW_LIQUIDITY_V2) && msg_value < Constants.DIRECT_PAIR_OP_MIN_VALUE_V2 + N_COINS * deploy_wallet_grams + referrer_value) {
    ...
    function depositLiquidity(
        ...
        require(lp_supply != 0 || (_operations[0].amount > 0 && _operations[1].amount > 0), Errors.WRONG_LIQUIDITY);
        require(
    ...
    function withdrawLiquidity(
        ...
        require(operationsOpt.hasValue(), Errors.WRONG_LIQUIDITY);

        TokenOperation[] operations = operationsOpt.get();
        _sync();

        IPairOperationCallback(account_owner)
    ...
    function exchange(
        ...
        IPairOperationCallback(account_owner).dexPairExchangeSuccess{
    ...
    function crossPoolExchange(
        ...
        if (op == OperationTypes.CROSS_PAIR_EXCHANGE && next_steps.length > 0) {
            // actually poolRoot is a tokenRoot here, so
        ...
        if (errorCode == 0) {
            ...
            if (dy_result.amount < expected_amount) {
               ...
               _sync();
                ...
                IPairOperationCallback(sender_address).dexPairExchangeSuccess{
                    value: Constants.OPERATION_CALLBACK_BASE + 4,
                    flag: MsgFlag.SENDER_PAYS_FEES + MsgFlag.IGNORE_ERRORS,
                    bounce: false
                }(id, false, IExchangeResult.ExchangeResult(
                    i == 0 && j == 1,
    ...
    function upgrade(TvmCell code, uint32 new_version, uint8 new_type, address send_gas_to) override external onlyRoot {
        if (current_version == new_version && new_type == PoolTypes.STABLESWAP) {
            tvm.rawReserve(Constants.PAIR_INITIAL_BALANCE, 0);
            send_gas_to.transfer({ value: 0, flag: MsgFlag.ALL_NOT_RESERVED + MsgFlag.IGNORE_ERRORS, bounce: false });
        } else {
            if (fee.beneficiary.value != 0) {
            ...
            builder.store(PoolTypes.STABLESWAP);
    ...
    function onCodeUpgrade(TvmCell upgrade_data) private {
        ...
        address send_gas_to;
        uint32 old_version;
        uint8 old_pool_type = PoolTypes.CONSTANT_PRODUCT;
        ...
        if (old_version == 0) {
            ...
            ITokenRoot(type_to_root_addresses[AddressType.RESERVE][0]).decimals{
                ...
                flag: MsgFlag.SENDER_PAYS_FEES,
                callback: StablePair.onTokenDecimals
            }();
    ...
    function _configureToken(address token_root) private view {
        ITokenRoot(token_root).deployWallet {
            ...
            flag: MsgFlag.SENDER_PAYS_FEES,
            callback: StablePair.onTokenWallet
